...
async function handleHostScene(interaction, member) {
    ...
    for (let i = 1; i <= 4; i++) {
        ...
        if (participant && participant instanceof discord_js_1.GuildMember) {
    ...
    try {
        const { scene, thread } = await scene_manager_1.sceneManager.createScene(channel, member, title, description || undefined, participants);
...
async function handleEndScene(interaction, member) {
    ...
    await interaction.deferReply();
    try {
        const scene = await scene_manager_1.sceneManager.endScene(interaction.channel.id, member.id, summary || undefined);
        const embed = new discord_js_1.EmbedBuilder()
            .setColor(0x8B4513)
...
async function handlePassHost(interaction, member) {
    ...
    if (!newHost || !(newHost instanceof discord_js_1.GuildMember)) {
