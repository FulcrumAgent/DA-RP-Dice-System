...
class CharacterCreationStateManager {
    ...
    setState(userId, guildId, updates) {
        ...
        if (this.dataManager && typeof this.dataManager.set === 'function') {
            this.dataManager.set(`char_creation:${key}`, updatedState, this.TTL_MS / 1000)
                .catch((err) => {
    ...
    async updateState(userId, guildId, updates) {
        ...
        const { data: _, tempData: __, ...otherUpdates } = updates;
