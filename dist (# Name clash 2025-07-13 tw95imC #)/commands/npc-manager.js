...
async function handleEditNPC(interaction, member) {
    ...
    try {
        ...
        const updatedNPC = await character_manager_1.characterManager.updateNPC(npc.id, field, value, member.id);
        await interaction.editReply({
...
async function handleSaveGeneratedNPC(interaction, member) {
...
async function handleSaveNPCNameModal(interaction, member) {
    ...
    try {
        ...
        const npc = await character_manager_1.characterManager.createNPC(npcName, interaction.guild.id, [concept], `Auto-generated ${tier} NPC with concept: ${concept}`, member.id, {
            tier: tier,
