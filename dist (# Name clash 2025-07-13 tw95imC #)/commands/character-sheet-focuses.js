...
async function handleFocusesSelect(interaction, member) {
    try {
        ...
        const panel = await buildFocusPanel(member, state, interaction.guildId);
        if (!panel) {
...
async function buildFocusPanel(member, state, guildId) {
    try {
        if (!state.data) {
        ...
        const usedFocuses = new Set(currentFocuses.map(([_, f]) => f));
        ...
        if (currentFocuses.length > 0) {
            const validFocuses = currentFocuses.filter(([s, f]) => f);
...
async function handleFocusSelect(interaction, member) {
    try {
        ...
        const skillsNeedingFocus = Object.entries(currentSkills).reduce((count, [_, value]) => {
            const skillValue = typeof value === 'number' ? value : 0;
