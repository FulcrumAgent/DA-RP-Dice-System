...
export class CharacterManager {
    ...
    try {
      ...
      if (charactersData) {
        charactersData.forEach(char => {
          ...
          // Legacy data migration - remove guildId references
          if ('guildId' in char) {
            delete (char as any).guildId;
          }
   ...
   */
  getUserActiveCharacter(userId: string): DuneCharacter | undefined {
   ...
   */
  getUserCharacters(userId: string): DuneCharacter[] {
    const userCharacters: DuneCharacter[] = [];
    for (const character of this.characters.values()) {
      if (character.userId === userId) {
        // Legacy data migration - remove guildId references
        if ('guildId' in character) {
          delete (character as any).guildId;
        }
  ...
  /**
   * Search for assets across all characters and NPCs
   */
  async searchAssets(query: string): Promise<Array<{ name: string; type: string; description: string; source: string }>> {
  ...
  /**
   * Get all characters (no longer guild-specific due to database unification)
   */
  async getAllCharacters(): Promise<DuneCharacter[]> {
